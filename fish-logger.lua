...
return string.format("X %.0f | Y %.0f | Z %.0f",p.X,p.Y,p.Z)
...
local id = tool.TextureId:gsub("%D","")
...
local id = mesh.TextureId:gsub("%D","")
return "https://www.roblox.com/asset-thumbnail/image?assetId="..id.."&width=420&height=420&format=png"
end
end
...
for _,v in pairs(EVENT_FISH) do
...
{name="Weight",value=tostring(data.weight).." kg",inline=true},
...
print("Fish Logger Started")

end

FishLogger:start()

function logFish(name,weight,mutation,rarity)
